...
C<--config> option, gdrive-sync uses the file called F<.gdsconfig> in the users
home directory if it exists.  This configuration file is run as a Bash script,
which allows for a somewhat dynamic configuration process.
...
Mandatry.  Use F<DIR> as the destination directory.  Config file option
F<dest_dir>.
...
Shawn M. Chapla - L<https://github.com/shwnchpl>

=head1 COPYRIGHT

Copyright (C) 2018 Shawn M. Chapla.

License GPLv2+: GNU GPL version 2 or later L<https://www.gnu.org/licenses/gpl.html>.  This is free softare: you are free to change and redistribute it.  There is NO WARRANTY, to the extent permitted by law.
